...
public class ReadOnly extends BaseTest {
    ...
    public void testReadOnlyQueue() throws JMSException, NamingException, QmfException {
        ...
        try (AutoCloseableConnection receiverConnection = this.utils.getConnectionBuilder().username(USER1_USERNAME).password(USER1_PASSWORD).connectionOption("sync_ack='true'").build()) {
