...
public class Security extends BaseTest {
    ...
    public void testAuthenticationPlain() throws JMSException, NamingException, InterruptedException {
        try (AutoCloseableConnection connection = this.utils.getConnectionBuilder().username(USER1_USERNAME).password(USER1_PASSWORD).brokerOption("sasl_mechs='PLAIN'").build()) {
            connection.start();
            Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
    ...
    public void testAuthenticationCramMD5() throws JMSException, NamingException, InterruptedException {
        try (AutoCloseableConnection connection = this.utils.getConnectionBuilder().username(USER1_USERNAME).password(USER1_PASSWORD).brokerOption("sasl_mechs='CRAM-MD5'").build()) {
            connection.start();
            Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
    ...
    public void testAuthenticationAnonymous() throws JMSException, NamingException, InterruptedException {
        try (AutoCloseableConnection connection = this.utils.getConnectionBuilder().brokerOption("sasl_mechs='ANONYMOUS'").build()) {
            connection.start();
            Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
    ...
    public void testACLDeniedProducerForbiddenTopic() throws JMSException, NamingException, InterruptedException {
        try (AutoCloseableConnection connection = this.utils.getConnectionBuilder().username(USER1_USERNAME).password(USER1_PASSWORD).build()) {
            ...
            MessageProducer sender = session.createProducer(this.utils.getTopic(FORBIDDEN_TOPIC, RTG_ROUTING_KEY));
    ...
    public void testACLDeniedProducerForbiddenRoutingKey() throws JMSException, NamingException, InterruptedException {
        try (AutoCloseableConnection connection = this.utils.getConnectionBuilder().username(USER1_USERNAME).password(USER1_PASSWORD).connectionOption("sync_publish='all'").build()) {
            connection.start();
            Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);

            MessageProducer sender = session.createProducer(this.utils.getTopic(RTG_TOPIC, FORBIDDEN_ROUTING_KEY));
            Message msg = session.createMessage();
            sender.send(msg);
        }
    }
