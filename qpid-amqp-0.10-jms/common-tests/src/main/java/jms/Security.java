...
public class Security extends BaseTest {
    ...
    public void testAuthenticationDefault() throws JMSException, NamingException, InterruptedException {
        try (AutoCloseableConnection connection = Utils.getConnectionBuilder().username(USER1_USERNAME).password(USER1_PASSWORD).build()) {
            connection.start();
    ...
    public void testAuthenticationPlain() throws JMSException, NamingException, InterruptedException {
        try (AutoCloseableConnection connection = Utils.getConnectionBuilder().username(USER1_USERNAME).password(USER1_PASSWORD).brokerOption("sasl_mechs='PLAIN'").build()) {
            connection.start();
            Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
    ...
    public void testAuthenticationCramMD5() throws JMSException, NamingException, InterruptedException {
        try (AutoCloseableConnection connection = Utils.getConnectionBuilder().username(USER1_USERNAME).password(USER1_PASSWORD).brokerOption("sasl_mechs='CRAM-MD5'").build()) {
            connection.start();
            Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
    ...
    public void testAuthenticationAnonymous() throws JMSException, NamingException, InterruptedException {
        try (AutoCloseableConnection connection = Utils.getConnectionBuilder().brokerOption("sasl_mechs='ANONYMOUS'").build()) {
            connection.start();
            Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
    ...
    public void testWrongPassword() throws JMSException, NamingException, InterruptedException {
        try (AutoCloseableConnection connection = Utils.getConnectionBuilder().username(USER1_USERNAME).password("wrongpassword").build()) {
            connection.start();
            Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
    ...
    public void testWrongUsername() throws JMSException, NamingException, InterruptedException {
        try (AutoCloseableConnection connection = Utils.getConnectionBuilder().username("nonexistentuser").password("mypassword").build()) {
            connection.start();
            Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
    ...
    public void testACLDeniedProducerForbiddenTopic() throws JMSException, NamingException, InterruptedException {
        try (AutoCloseableConnection connection = Utils.getConnectionBuilder().username(USER1_USERNAME).password(USER1_PASSWORD).build()) {
            ...
            MessageProducer sender = session.createProducer(Utils.getTopic(FORBIDDEN_TOPIC, RTG_ROUTING_KEY));
    ...
    public void testACLDeniedProducerForbiddenRoutingKey() throws JMSException, NamingException, InterruptedException {
        try (AutoCloseableConnection connection = Utils.getConnectionBuilder().username(USER1_USERNAME).password(USER1_PASSWORD).connectionOption("sync_publish='all'").build()) {
            connection.start();
            Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);

            MessageProducer sender = session.createProducer(Utils.getTopic(RTG_TOPIC, FORBIDDEN_ROUTING_KEY));
            Message msg = session.createMessage();
            sender.send(msg);
        }
    }
    ...
    public void testMaximumAllowedConnections() throws JMSException, NamingException, InterruptedException {
        try (AutoCloseableConnection connection = Utils.getConnectionBuilder().username(USER2_USERNAME).password(USER2_PASSWORD).build();
                AutoCloseableConnection connection2 = Utils.getConnectionBuilder().username(USER2_USERNAME).password(USER2_PASSWORD).build();
                AutoCloseableConnection connection3 = Utils.getConnectionBuilder().username(USER2_USERNAME).password(USER2_PASSWORD).build();
                AutoCloseableConnection connection4 = Utils.getConnectionBuilder().username(USER2_USERNAME).password(USER2_PASSWORD).build();
                AutoCloseableConnection connection5 = Utils.getConnectionBuilder().username(USER2_USERNAME).password(USER2_PASSWORD).build()) {
            connection.start();
            Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
            connection2.start();
            ...
            try (AutoCloseableConnection connection6 = Utils.getConnectionBuilder().username(USER2_USERNAME).password(USER2_PASSWORD).build()) {
                connection6.start();
