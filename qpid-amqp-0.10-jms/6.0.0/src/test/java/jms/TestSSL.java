...
public class TestSSL {
    ...
    public void testHostnameVerification() throws JMSException, NamingException, InterruptedException {
        ...
        try {
            Connection connection = Utils.getSSLConnectionBuilder().hostname(IP_ADDRESS).keystore(USER1_KEYSTORE).keystorePassword(USER1_KEYSTORE_PASSWORD).keystoreAlias(USER1_KEYSTORE_ALIAS).build();
            connection.start();
            Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
        ...
        try {
            Connection connection = Utils.getSSLConnectionBuilder().hostname(IP_ADDRESS).keystore(USER1_KEYSTORE).keystorePassword(USER1_KEYSTORE_PASSWORD).keystoreAlias(USER1_KEYSTORE_ALIAS).brokerOption("ssl_verify_hostname='false'").build();
            connection.start();
            Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
    ...
    public void testPlainOverSSLWithClientAuth() throws JMSException, NamingException, InterruptedException {
        Connection connection = Utils.getSSLConnectionBuilder().keystore(USER1_KEYSTORE).keystorePassword(USER1_KEYSTORE_PASSWORD).keystoreAlias(USER1_KEYSTORE_ALIAS).username(ADMIN_USERNAME).password(ADMIN_PASSWORD).brokerOption("sasl_mechs='PLAIN'").build();
        connection.start();
        Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
    ...
    public void testPlainOverSSL() throws JMSException, NamingException, InterruptedException {
        Connection connection = Utils.getSSLConnectionBuilder().username(ADMIN_USERNAME).password(ADMIN_PASSWORD).brokerOption("sasl_mechs='PLAIN'").build();
        connection.start();
        Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
    ...
    public void testAnonymousOverSSLWithClientAuth() throws JMSException, NamingException, InterruptedException {
        Connection connection = Utils.getSSLConnectionBuilder().keystore(USER1_KEYSTORE).keystorePassword(USER1_KEYSTORE_PASSWORD).keystoreAlias(USER1_KEYSTORE_ALIAS).brokerOption("sasl_mechs='ANONYMOUS'").build();
        connection.start();
        Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
