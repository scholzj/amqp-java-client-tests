...
public class TestSecurity {
    ...
    public void testAuthenticationPlain() throws JMSException, NamingException, InterruptedException {
        Connection connection = Utils.getConnectionBuilder().username(USER1_USERNAME).password(USER1_PASSWORD).brokerOption("sasl_mechs='PLAIN'").build();
        connection.start();
        Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
    ...
    public void testAuthenticationCramMD5() throws JMSException, NamingException, InterruptedException {
        Connection connection = Utils.getConnectionBuilder().username(USER1_USERNAME).password(USER1_PASSWORD).brokerOption("sasl_mechs='CRAM-MD5'").build();
        connection.start();
        Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
    ...
    public void testAuthenticationAnonymous() throws JMSException, NamingException, InterruptedException {
        Connection connection = Utils.getConnectionBuilder().brokerOption("sasl_mechs='ANONYMOUS'").build();
        connection.start();
        Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
    ...
    public void testWrongUsername() throws JMSException, NamingException, InterruptedException {
        Connection connection = Utils.getConnectionBuilder().username("nonexistentuser").password("mypassword").build();
        connection.start();
        Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
    ...
    public void testACLDeniedProducerForbiddenRoutingKey() throws JMSException, NamingException, InterruptedException {
        Connection connection = Utils.getConnectionBuilder().username(USER1_USERNAME).password(USER1_PASSWORD).connectionOption("sync_publish='all'").build();
        connection.start();
        Session session = connection.createSession(false, Session.CLIENT_ACKNOWLEDGE);
